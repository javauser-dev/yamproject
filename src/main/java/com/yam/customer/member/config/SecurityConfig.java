...
public class SecurityConfig {
	...
	public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
		http.csrf((csrf) -> csrf.disable()) // 테스트를 위해 csrf 보호 비활성화. 프로덕션에서는 사용 X
				.authorizeHttpRequests((authz) -> authz.anyRequest().permitAll() // 모든 URL 허용 (*테스트용*)
				)
				.formLogin((formLogin) -> formLogin.loginPage("/customer/login").usernameParameter("customerId")
