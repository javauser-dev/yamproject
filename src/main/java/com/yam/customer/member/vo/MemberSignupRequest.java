...
//    @Pattern(regexp = "^(?=.*[a-zA-Z])(?=.*[0-9])(?=.*[@$!%*?&])[a-zA-Z0-9@$!%*?&]{8,15}$", message = "비밀번호는 영문자, 숫자, 특수문자를 포함하여 8~15글자여야 합니다.")
...
//    private String customerPassword;
    ...
    @NotBlank(message = "비밀번호는 필수 입력값입니다.")
    @Pattern(regexp = "^(?=.*[a-zA-Z])(?=.*\\d)(?=.*[@$!%*?&])[A-Za-z\\d@$!%*?&]{8,15}$", message = "비밀번호는 영문자, 숫자, 특수문자를 포함하여 8~15글자여야 합니다.")
    private String customerPassword;

    @NotBlank(message = "닉네임은 필수 입력값입니다.")
    @Pattern(regexp = "^[a-zA-Z0-9ㄱ-ㅎㅏ-ㅣ가-힣]{2,8}$", message = "닉네임은 영문 대소문자, 숫자, 한글 2~8글자여야 합니다.")
