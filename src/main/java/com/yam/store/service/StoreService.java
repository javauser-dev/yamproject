...
public class StoreService {
	...
	public void registerStore(StoreDTO storeDTO) {
		...
		Store store = Store.builder().name(storeDTO.getName()).email(storeDTO.getEmail())
				.password(passwordEncoder.encode(storeDTO.getPassword())) // ✅ 비밀번호 암호화
				.phone(storeDTO.getPhone()).businessNumber(storeDTO.getBusinessNumber()).agree(storeDTO.isAgree())
				.enabled(false).verificationToken(UUID.randomUUID().toString()).build();
	...
	public String login(String email, String password) {
		Store store = storeRepository.findByEmail(email).orElseThrow(() -> new RuntimeException("사업자 정보를 찾을 수 없습니다."));
	...
	public Store findByBusinessNumber(String businessNumber) {
		return storeRepository.findByBusinessNumber(businessNumber).orElse(null); // 없으면 null 반환
	}
