		...
		$.ajax({
			...
			if(resultData=="불일치"){
				...
				if(buttonCheck=="updateButton"){
					actionProcess("#dataForm", "get", "/storeCommunity/updateForm");
				}else if(buttonCheck == "deleteButton"){
					if(confirm("정말 삭제하시겠습니까?")){
						actionProcess("#dataForm", "post", "/storeCommunity/boardDelete");
					}
				}
			}
		}).fail(function(){
...
$("#boardListBtn").on("click", function(){
	locationProcess("/storeCommunity/boardList");
});

/* 수정 버튼 클릭 시 처리 이벤트 */
$("#updateFormBtn").click(function(){	
	actionProcess("#dataForm", "post", "/storeCommunity/updateForm");
});
